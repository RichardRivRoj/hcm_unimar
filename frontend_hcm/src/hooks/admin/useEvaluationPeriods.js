...
const useEvaluationPeriods = () => {
  ...
  const createEvaluationPeriod = useCallback(async (formData) => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await axios.post('/api/admin/evaluation-periods', formData, {
        headers: {
  ...
  const updateEvaluationPeriod = useCallback(async (id, formData) => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await axios.put(`/api/admin/evaluation-periods/${id}`, formData, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
  ...
  const deleteEvaluationPeriod = useCallback(async (id) => {
    try {
        setLoading(true);
        setError(null);
