import {
  ...
  const SelectionResultPDF = ({ result }) => {
    const formatDate = (dateString) => {
        // Si ya está en el formato correcto, devolverlo directamente
        if (dateString.match(/\d{1,2} de \w+ de \d{4}/)) {
          return dateString;
        }
        
        // Si es una fecha ISO (YYYY-MM-DD), formatear
      ...
      <Document>
        <Page size="A4" style={styles.page}>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>I. DATOS DEL POSTULANTE</Text>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>II. RESULTADOS DE EVALUACIÓN</Text>
            <View style={styles.evaluationTable}>
              ...
              {result.process_details.agendas.map((agenda, index) => (
                <View key={index} style={styles.tableRow}>
                  <Text style={styles.tableCell}>{agenda.type}</Text>
                  <Text style={styles.tableCell}>{agenda.scheduled_date}</Text>
                  <Text style={styles.tableCell}>{agenda.score}</Text>
                </View>
              ))}
            </View>
            ...
            <View style={styles.row}>
              ...
              <View style={styles.column}>
                <Text style={styles.label}>Total de Evaluaciones:</Text>
                <Text style={styles.value}>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>III. CRONOGRAMA DEL PROCESO</Text>
            {result.process_details.timeline.map((event, index) => (
              <View key={index} style={styles.timelineItem}>
                <View style={styles.timelineDot} />
                <View>
                  <Text style={{ fontWeight: 'bold' }}>{event.event}</Text>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.value}>
              El presente informe certifica que el proceso de selección se ha
              realizado conforme a los lineamientos establecidos en el Reglamento
              de Personal de la Universidad de Margarita. Los resultados obtenidos
              son definitivos y forman parte del expediente del candidato.
            </Text>
          </View>
          ...
          <View style={styles.signatureSection}>
            ...
            <View style={styles.signatureBox}>
              <Text style={styles.label}>DIRECCIÓN DE TALENTO HUMANO</Text>
              <View style={styles.signatureLine} />
              <Text style={styles.valuetwo}>Nombre y Firma</Text>
            </View>
          </View>
          ...
          <View style={styles.footer}>
            ...
            <Text>Universidad de Margarita - Todos los derechos reservados</Text>
          </View>
        </Page>
      </Document>
    );
