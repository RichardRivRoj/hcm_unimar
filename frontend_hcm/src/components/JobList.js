            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                {jobPostings.map((job) => (
                    <div key={job.id} className="p-4 bg-white rounded shadow">
                        ...
                        <p className="mt-2 text-sm">{truncateDescription(job.description)}</p>
                        <p className="mt-2 text-sm font-medium">
                            {job.isRemote ? 'Trabajo Remoto' : 'Presencial'}
                        </p>
                        <button
                            onClick={() => handleApply(job.id)}
                            className="px-4 py-2 mt-4 text-white bg-[#004b9a] rounded shadow hover:bg-blue-700 text-sm"
                        >
                            Postularse
                        </button>
                    </div>
            ...
            <div className="flex items-center justify-center mt-6 space-x-2">
                <button
                    ...
                    className={`px-2 py-1 text-xs text-white rounded ${
                        currentPage === 1 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#004b9a] hover:bg-blue-700'
                    }`}
                >
                    ← Anterior
                </button>
                ...
                <button
                    ...
                    className={`px-2 py-1 text-xs text-white rounded ${
                        currentPage === totalPages ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#004b9a] hover:bg-blue-700'
                    }`}
                >
                    Siguiente →
                </button>
