...
} = require('@/hooks/employee/useEmployeeTrainings')
...
const TrainingProgramDetail = ({ params }) => {
    ...
    if (programDetails.error)
        return (
            <Alert>
                <AlertDescription>{programDetails.error}</AlertDescription>
            </Alert>
        )
    ...
    return (
        <div className="px-4 py-8 mx-auto max-w-7xl">
            {programDetails.data?.classification === 'PUBLICO' && (
                <PublicProgramDetail
                    program={programDetails.data}
                    onEnroll={handleEnroll}
                />
            )}
            {(programDetails.data?.classification === 'INSCRITOS' ||
                programDetails.data?.classification === 'EN_PROGRESO') && (
