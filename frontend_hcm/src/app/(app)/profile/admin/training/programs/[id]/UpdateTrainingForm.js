...
const UpdateTrainingProgramForm = ({ programId, onClose }) => {
    ...
    // Carga inicial de opciones y datos del programa
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [depts, emps, types, mods, program] = await Promise.all([
                    ...
                    fetchProgram(programId), // Obtener datos del programa
                ])
                ...
                // Actualizar el estado con los datos del programa
                if (program.success) {
                    setFormData({
                        ...program.program,
                        content: JSON.parse(program.program.content || '[]'),
                        departments:
                            program.program.departments?.map(d => d.id) || [], // Asegurar array
                        employees:
                            program.program.employees?.map(e => e.id) || [], // Asegurar array
                    })
                }
            } catch (error) {
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            const response = await updateProgram(programId, payload)
            ...
            if (response.success) {
                toast.success('Programa actualizado', {
                    ...
                    action: {
                        ...
                        onClick: () =>
                            router.push(
                                `profile/admin/training/programs/${response.program.id}`,
                            ),
                    },
                })
                onClose()
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={e => {
                                    const newVisibility = parseInt(
                                        e.target.value,
                                    )
                                    setFormData(prev => ({
                                        ...prev,
                                        visibility_id: newVisibility,
                                        // Resetear solo si cambia la visibilidad
                                        departments:
                                            newVisibility === 3
                                                ? prev.departments
                                                : [],
...
export default UpdateTrainingProgramForm
