...
const PeriodForm = ({ onClose, onSuccess }) => {
    const { createEvaluationPeriod } = useEvaluationPeriods()
    const {
        register,
        handleSubmit,
        setError,
    ...
    const onSubmit = async data => {
      try {
          ...
          // Removed console.error and kept the error handling logic
          if (error.errors) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-6">
            ...
            <div className="flex justify-end space-x-4">
                ...
                <Button
                    type="submit"
                    variant="primary"
                    className="text-white bg-[#004b9a] hover:bg-[#003366] focus:bg-[#002244] active:bg-[#001122]">
                    Crear Período
                </Button>
            </div>
        </form>
